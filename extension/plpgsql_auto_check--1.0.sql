...
begin
  ...
  for vlevel, vmessage in
      select level,
       ...
       cross join lateral (
                  select substr(query, 1, f.position + shift_to_rigth-1)
                         || e'\n'
                         || repeat('-', shift_to_left) || '^'
                         || substr(query, f.position + shift_to_rigth)
                    from substr(query, f.position)
                   cross join coalesce(
                  ...
                  ) as fq(formatted_query)
       cross join format(
                    e'plpgsql_check (%s): %s (sqlstate=%s)\nFunction: %s (line=%s, statement=%s)%s%s%s%s%s',
                    f.level, f.message, f.sqlstate,
                    p.oid::regproc, f.lineno, f.statement,
                    coalesce(e'\nTrigger: ' || p.tgname || ' on ' || p.tgrelid::regclass, ''),
                    coalesce(e'\nQuery: ' || fq.formatted_query, ''),
                    coalesce(e'\nDetail: ' || f.detail, ''),
                    coalesce(e'\nHint:' || f.hint, ''),
                    coalesce(e'\nContext: ' || f.context, '')) as fm(final_message)
       where l.lanname = 'plpgsql' and
             (f.sqlstate <> all(vexclude_sqlstates)
              or
              vexclude_sqlstates is null) and
             (fm.final_message !~ vexclude_message_pattern
              or
              vexclude_message_pattern is null)
  loop
    if vlevel = 'error' and
       current_setting('plpgsql_auto_check.on_error', true) = 'error'
    then
      raise exception '%', vmessage;
    else
      raise warning '%', vmessage;
    end if;
  end loop;
end;
$$ language plpgsql;
